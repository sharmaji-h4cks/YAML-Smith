...
import { CICDPipelineGenerator } from '../services/cicdGenerator';
import { CICDPipelineInput } from '../schemas/cicdSchemas';
...
export const generateCICDPipeline = async (
  ...
  try {
    const input: CICDPipelineInput = req.body;
    const generator = new CICDPipelineGenerator(input);
...
export const validateCICDPipeline = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
...
export const getCICDTemplates = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
