...
import {
  ...
  CICDPipelineOutput,
  CICDValidation,
} from '../schemas/cicdSchemas';

export class CICDPipelineGenerator {
  private input: CICDPipelineInput;
  ...
  generate(): CICDPipelineOutput {
    const output: CICDPipelineOutput = {};
  ...
  private generateGitHubActions(): string {
    ...
    return yaml.dump(workflow, { lineWidth: 120, quotingType: '"' });
  }
  ...
  private generateGitLabCI(): string {
    ...
    return yaml.dump(pipeline, { lineWidth: 120, quotingType: '"' });
  }
  ...
  private generateReadme(): string {
    ...
    if (this.input.test?.enabled) lines.push('- ✅ Automated testing');
    if (this.input.security?.enabled) lines.push('- ✅ Security scanning');
    lines.push('- ✅ Docker image build and push');
    if (this.input.deployment?.enabled) lines.push('- ✅ Kubernetes deployment');
    lines.push('');
    ...
    lines.push('---');
    lines.push('');
    lines.push('*Generated by YAML Smith*');

    return lines.join('\n');
  }

  // Helper methods
  private getGitHubTriggers(): any {
    const triggers: any = {};
  ...
  private getLanguageSetup(platform: string): any {
  ...
  private getLanguageImage(): string {
  ...
  private getTestSteps(platform: string): any {
  ...
  private getRegistryLogin(platform: string): any {
  ...
  private getImageTags(commitSha: string): string[] {
    const tags: string[] = [];
  ...
  private getDeploymentSteps(platform: string): any {
    ...
    switch (this.input.deployment?.strategy) {
      ...
      case 'helm':
        if (this.input.deployment.helmChart) {
          commands.push(
            `helm upgrade --install ${this.input.deployment.helmChart.name} ${this.input.deployment.helmChart.path} ` +
  ...
  validate(): CICDValidation {
    const warnings: CICDValidation['warnings'] = [];
