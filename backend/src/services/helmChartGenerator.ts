...
import {
  ...
  HelmChartOutput,
  HelmChartValidation,
} from '../schemas/helmChartSchemas';

export class HelmChartGenerator {
  private input: HelmChartInput;
  ...
  private generateChartYaml(): string {
    ...
    return yaml.dump(chart, { lineWidth: 120, quotingType: '"' });
  }
  ...
  private generateValuesYaml(): string {
    ...
    return yaml.dump(values, { lineWidth: 120, quotingType: '"' });
  }
...
`;
  ...
  private generatePodDisruptionBudgetTemplate(): string {
    const chartName = this.input.metadata.name;
...
`;
  ...
  private generateServiceMonitorTemplate(): string {
    const chartName = this.input.metadata.name;
...
`;
  ...
  private generateNetworkPolicyTemplate(): string {
    const chartName = this.input.metadata.name;
...
`;
  ...
  private generateTestConnection(): string {
    const chartName = this.input.metadata.name;
...
`;
  ...
  private generateEnvironmentValues(env: any): string {
    const envValues: any = {
    ...
    return yaml.dump(envValues, { lineWidth: 120, quotingType: '"' });
  }

  validate(): HelmChartValidation {
    const warnings: HelmChartValidation['warnings'] = [];
